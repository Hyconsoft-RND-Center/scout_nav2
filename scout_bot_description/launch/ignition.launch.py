...
def generate_launch_description():
    ...
    # wall_model_path = os.path.join(pkg_scout_bot_description, 'models', 'height_map', 'model.sdf')
    # wall_model_path = os.path.join(pkg_scout_bot_description, 'models', 'depot', '6', 'model.sdf')
    spawn_wall = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=[
    ...
    spawn_robot = Node(
        ...
        arguments=[
            ...
            '-x', '-4.0',
            '-y', '-0.0',
            '-z', '0.66' # base_link 기준이므로 높이 0에서 소환하는 경우 타이어가 땅에 박힐 수 있음
        ],
        output='screen'
    )
    ...
    controller_manager_node = Node(
        ...
        remappings=[("scout_drive_controller/odom", "odom")],
        output="screen",
    )
